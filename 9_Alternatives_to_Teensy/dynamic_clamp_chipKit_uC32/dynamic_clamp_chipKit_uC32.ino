// teensy 3.6 sketch to implement dynamic clamp on a ChipKit uC32 with an Analog Shield
//
...
//            by Processing, Matlab, Igor Pro, Python, or whatever). Any numbers that should be pre-calculated
//            and stored in global variables (e.g., the lookup table numbers for sodium activation/inactivation) 
//            are calculated by calling the function (in one of the tabbed files) that defines them. The trigger pin
//            for EPSCs is established.
...
//            part reads the signal from the patch clamp amplifier's output (which is read directly from the
//            Analog Shield), converts this signal into membrane potential v in millivolts (using the calibration parameters),
//            and sends v out to the functions (in the tabbed files) that calculate the dynamic clamp current in picoamps
//            for the various conductances. All of these currents are summed. The third and last part converts the 
//            summed current (in picoamps) into the output to be sent from the analog output pin. This output will
//            added to the DAQ board's current clamp command by breadboard electronics. The total signal will then be fed to 
...
//
// Last revised 08/25/17 at 12:11 am


#include <math.h>
#include <analogShield.h>   // library for the Analog Shield
#include <SPI.h>            // required for ChipKIT
...
const float inputSlope = 0.155/inputScaling;      
const float inputIntercept = -5000/inputScaling;    
const float outputSlope = 6553.6/outputScaling;       
const float outputIntercept = 32768; 
...
float t0 = micros();              // microsec, t0 is used to calculate dt on every iteration
float epscTime = millis();        // msec, time at which the last EPSC was triggered 
float epscTimeLapsed = 0.0;       // msec, time since the last EPSC was triggered

// Hardware connections
const int analogInPin = 0;        // uC32 AI pin used to read membrane potential
const int analogOutPin = 0;       // uC32 AO pin used to output current
const int epscTriggerPin = 13;    // pin used to trigger EPSC train 
...
void loop() {
  ...
  float v = inputSlope * analog.read(analogInPin) + inputIntercept;   // mV, given amplifier settings 
  float injectionCurrent = 0.0;                                       // pA
  dt = 0.001*(micros()-t0);                                           // msec, time step                                 
  t0 = micros();                                                      
  ...
  if (gEPSC>0) {
    epscTimeLapsed = millis()-epscTime;
    if ((digitalRead(epscTriggerPin)==HIGH)&&(epscTimeLapsed>2)) {        // poll epscTriggerPin to see if a trigger has arrived
      epscTime = millis();
      UpdateEpscTrain();
    }
  ...
  int outputSignal = constrain((int)injectionCurrent,0,65535);           // make sure the output is an integer between 0 and 65535 (16 bits)
  analog.write(analogOutPin,outputSignal);                               // send the output to the patch clamp or summing amplifier
