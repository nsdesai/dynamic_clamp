...
float alphaM[151] = {0.0};                          // Pre-calculate activation and inactivation parameters 
float betaM[151] = {0.0};                           // for sodium currents: Vm from -100 mV to +50 mV in 
float alphaH[151] = {0.0};                          // steps of 1 mV
float betaH[151] = {0.0};
...
void GenerateSodiumLUT() {
  float v;
  for (int x=0; x<151; x++) {
    v = (float)x - 100.0;                          // We use membrane potentials between -100 mV and +50 mV
    if (x==60) {                                   // The if-else statement makes sure alphaM stays finite at v = -40 mV.
      alphaM[x] = 1.0;
    } else {
      alphaM[x] = 0.1 * (-(v+40.0)) / ( exp( -(v+40.0)/10 ) - 1 );
    }
    betaM[x] = 4.0 * exp(-(v+65)/18);
    alphaH[x] = 0.07 * exp(-(v+65)/20);
    betaH[x] = 1 / ( exp(-(v+35)/10 ) + 1); 
  }
}
